...
export default async function RootLayout({children}:{children:React.ReactNode}){const {products,demo}=await getCatalog();return <html lang="pt-BR"><body><StoreProvider products={products} demo={demo}><RecoveryRedirect/><Header/>{children}<Footer/><CookieBanner/><WhatsApp number={process.env.WHATSAPP_NUMBER||''}/></StoreProvider></body></html>}
